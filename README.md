...
- Timeline reduz spam automaticamente: consolida movimentos e cliques para leitura/editing mais limpa (`X,Y` como foco).
- Playback filter com checkboxes: **Mouse moves**, **Mouse clicks**, **Key presses**, **Wait times** (na aba de inspeção).
- Filtros de playback também filtram a visualização: ao desmarcar um tipo (ex.: mouse moves), ele some da timeline e não executa no replay.
...
- Loop por variável CSV: defina nome (ex.: `periodo`) e valores (ex.: `2027/1,2027/2`, separados por vírgula, `;` ou quebra de linha) para executar a macro do zero em cada valor e parar automaticamente no final.
